...
do(study(X)):-study(X),!.
do(quit):-quit,!.
do(teleport(X)):-teleport(X),!.%teleport
do(toss(X)):-toss(X),!.
...
nshelp:-
  ...
  write('   turn something on     (ex. turn on light)'),nl,
  write('   turn something off    (ex. turn off light)'),nl,
  write('   check inventory       (ex. inventory)'),nl,
  write('   toss something        (ex. toss apple)'),nl,
  write('   teleport to place     (ex. teleport kitchen)'),nl,
  nl,
...
location(trolly,office).  %QUESTION8 if you take the trolly, you can take heavy items.
%QUESTION4END
...
can_go(Place,X):-
  ...
  connect(X,Place),
  turned_off(flashlight),
  write("You should turn on the light"),
  nl,
  fail. 
can_go(Place,X):-
  Place = cellar,
...
can_go(Place,X):-
  \+Place = cellar,
  Place = X,
  write("You can't go to the "),
  write(Place),
  write(" from here, you are already in there."),
  nl,
  fail.
...
toss(Thing):-
  ...
  tossable(Thing),
  retract(have(Thing)),
  respond(['you tossed the ',Thing]),
  !.
toss(Thing):-
  \+ have(Thing),
  respond(['you don''t have the ',Thing]).
toss(Thing):-
  respond(['You can''t toss the ',Thing]).
%QUESTION7END
